    ...
    def __call__(self, keypoints):
        ...
        for p in keypoints:
            for idx, pt in enumerate(p):
                if pt[0] > 0: # 키 포인트 x좌표가 0보다 큰 지 확인 
                    x, y = int(pt[0]), int(pt[1])
                    if x<0:
                        x = 0
                    elif x>=self.output_res:
                        x = self.output_res-1
                    if y<0:
                        y = 0
                    elif y>=self.output_res:
                        y = self.output_res-1
...
class Dataset(torch.utils.data.Dataset):
    ...
    def loadImage(self, idx):
        ...
        transformed = self.transform(image=cv2.resize(orig_img, (64,64)), keypoints=keypoints)
        ...
        heatmaps = generateHeatmap(np.array(transformed['keypoints']).reshape(1, -1, 2))
        orig_img = cv2.resize(transformed['image'], (256, 256)).reshape(1, 256, 256) / 255.0
        meta= {'index' : idx}
