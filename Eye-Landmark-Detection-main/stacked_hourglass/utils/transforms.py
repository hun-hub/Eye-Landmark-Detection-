...
def transform(pt, res, invert=0, rot=0):
    # Transform pixel location to different reference
    t = get_transform(res, rot=rot)
    if invert:
        t = np.linalg.inv(t)
...
def transform_preds(coords, res):
    for p in range(coords.size(0)):
        coords[p, 0:2] = to_torch(transform(coords[p, 0:2], res, 1, 0))
    return coords
